...
def content_filter(response: str) -> dict:
    ...
    PII_PATTERNS = {
        ...
        "known_admin_password": r"\badmin123\b",
    ...
    async def after_model_callback(
        ...
        *,
        ...
        if not filter_result["safe"]:
            ...
            filtered_text = filter_result["redacted"]
            llm_response.content = types.Content(
                role="model",
                parts=[types.Part.from_text(text=filtered_text)],
            )

        if self.use_llm_judge:
            judge_result = await llm_safety_check(filtered_text)
            if not judge_result["safe"]:
                self.blocked_count += 1
                llm_response.content = types.Content(
                    role="model",
                    parts=[types.Part.from_text(
                        text="I cannot provide that response. Please rephrase with a safe banking question."
                    )],
                )
